...
// uses, which came from wizard/panel/door.html), both themes, and the nav.
// The nav is NAV below rather than a number in this comment, because the last
// two things added to it left this line saying "four".
//
...
const NAV = [
  ...
  // Access rides SECOND (2026-08-13). It is the page that answers the question
  // the account exists to answer: who and what can reach my minerals. Devices
  // was folded into it and deleted the same day (Sam's ruling) — it had become
  // a second, machine-shaped view of the same roster, plus a session list that
  // belongs with sign-in, plus three fields that could never hold a value.
  { route: 'access', href: '/app/access', label: 'Access' },
  // "Activity" promised an archive and delivered a 30-day tie-endings feed. It
  // is a 30-day change feed and now says so (2026-08-13).
  { route: 'activity', href: '/app/activity', label: 'Recent changes' },
...
function renderAppNav(active, isAdmin) {
  ...
  // "Operator", matching the page's own heading. It said Admin in the nav and
  // Operator on the page, which is a small thing that makes a product feel like
  // two products (2026-08-13).
  const items = isAdmin ? [...NAV, { route: 'admin', href: '/app/admin', label: 'Operator' }] : NAV;
