...
function makeRedeemHandler({ kv }) {
  return async function handler(req, res) {
    ...
    return res.status(200).json({
      ok: true,
      email: user.email,
      session_jwt: signSession({ email: user.email, state_version: sv }),
      app_token: mintAppToken({ email: user.email, stateVersion: sv }),
...
function makeTokenHandler({ kv }) {
  return async function handler(req, res) {
    ...
    return res.status(200).json({
      ok: true,
      email: user.email,
      app_token: mintAppToken({ email: user.email, stateVersion: user.state_version || 1,
