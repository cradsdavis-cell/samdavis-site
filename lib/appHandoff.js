...
function makeRedeemHandler({ kv }) {
  return async function handler(req, res) {
    ...
    return res.status(200).json({
      ...
      app_token: mintAppToken({ email: user.email, stateVersion: sv, accountId: user.id, billingEnabled: !!user.has_card }),
...
function makeTokenHandler({ kv }) {
  return async function handler(req, res) {
    ...
    return res.status(200).json({
      ok: true,
      email: user.email,
      app_token: mintAppToken({ email: user.email, stateVersion: user.state_version || 1, accountId: user.id, billingEnabled: !!user.has_card,
