...
function billingView({ user = {}, minerals = [], events = [], prices = ZERO, bands = {}, now = Date.now(), baseUrl = 'https://crads-ai.com' } = {}) {
  ...
  for (const m of minerals) {
    ...
    if (m.tier === 'rock') {
      ...
      const monthly = (prices.tier || 0) + (prices.hosting || 0) * hosting;
      lines.push({
      ...
      lines.push({ kind: 'pebble-anchored', label: m.label || m.host, anchor: m.anchor, monthly: 0, so_far: 0, days_so_far: daysSoFar });
    }
  }
