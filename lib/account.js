...
const { loginUrlFor } = require('./safeNext');

async function requireAuth({ kv, req, res }) {
  // Where they were trying to go, so signing in returns them to it instead of
  // dumping them on the account page. api/app/access.js has claimed "then back
  // here" since June and nothing implemented it, so an emailed invitation was
  // lost the moment the grantee had to sign in (QA finding 46). safeNext.js
  // decides what is safe to honour; everything else falls back to a bare
  // /account/login, which is exactly the old behaviour.
  const bounce = () => { res.redirect(302, loginUrlFor(req && req.url)); return null; };

  const jwt = parseSessionFromRequest(req);
  if (!jwt) return bounce();
  const payload = verifySession(jwt);
  if (!payload) return bounce();
  const user = await kv.getUser(payload.email);
  if (!user) return bounce();
  if ((user.state_version || 1) !== (payload.state_version || 1)) return bounce();
