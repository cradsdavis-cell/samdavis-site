...
function normalizePem(raw) {
  ...
  // the reliable path: the WHOLE PEM base64-encoded into one line
  // (base64 -w0 key.pem). Single-line values survive every dashboard.
  if (!s.includes('-----BEGIN') && /^[\s"']*[A-Za-z0-9+/=\s"']+$/.test(s)) {
    try {
      const dec = Buffer.from(s.replace(/[\s"']+/g, ''), 'base64').toString('utf8');
      if (dec.includes('-----BEGIN')) s = dec;
...
function mintAppToken({ email, stateVersion, nonce, scope, accountId, billingEnabled }) {
  if (!email) throw new Error('mintAppToken needs an email');
  return jwt.sign({
    email,
    email_verified: true,
    sv: stateVersion || 1,
    // the account's permanent name (ownership ruling 5). Ownership and access
    // records key on THIS, not on the email hash, so changing an email never
    // orphans a mineral. Absent on tokens minted for accounts that have not
    // been loaded since the id shipped; consumers must tolerate that.
    ...(accountId ? { aid: String(accountId) } : {}),
    // THE BILLING CLAIM (2026-08-23 placeholder spec): the directory gates
    // creating, promoting, anchoring and receiving a mineral on this, read
    // from the token it already verifies, so the build path makes no call to
    // the site. The site stays the source of truth: a flip here is on the next
    // token, within the 1h life. Absent on tokens minted before the claim
    // shipped; the worker treats absent as false.
