...
function normalizePem(raw) {
  ...
  if (!s.includes('\n')) {
    // single-line paste: re-flow header + base64 body + footer
    const m = s.match(/^(-----BEGIN [A-Z ]+-----)\s*([\s\S]*?)\s*(-----END [A-Z ]+-----)$/);
    if (m) s = `${m[1]}\n${m[2].replace(/\s+/g, '').replace(/(.{64})/g, '$1\n').trim()}\n${m[3]}`;
  } else {
    // newlines survived but the body may carry stray spaces from wrapping
    const m = s.match(/^(-----BEGIN [A-Z ]+-----)\n([\s\S]*?)\n(-----END [A-Z ]+-----)$/);
    if (m) s = `${m[1]}\n${m[2].replace(/[ \t]+/g, '')}\n${m[3]}`;
  }
  return s;
}
