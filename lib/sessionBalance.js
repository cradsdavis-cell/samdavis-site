...
//
// v4 (2026-09-09): guided-setup is the live two-session engagement (session 1 at
// checkout, session 2 from the portal). The coaching-block entries stay so records
// written before the pivot keep computing; continuation-retainer stays as a STATE
// (retainer-active) for the same reason. Nothing here can sell either again.
'use strict';
...
const PORTAL_BOOKABLE_BLOCK_SKUS = new Set(['guided-setup', 'coaching-block', 'coaching-block-pay4']);

// Engagements whose session 2 must sit at least this long after session 1 (the
// guided setup's onboarding homework lives in that gap).
const MIN_GAP_MS_BY_SKU = { 'guided-setup': 24 * 60 * 60 * 1000 };
