...
// Where to send them once they are in. /account unless they were bounced here
// from somewhere else, in which case back to that (QA finding 46: an invitation link
// was lost through the sign-in redirect). The client sends `next`; the guard,
// not the client, decides whether it is honoured, so a crafted body cannot turn
// our sign-in into a redirector for somebody else's site.
function landing(req) {
  return safeNext(req && req.body && req.body.next) || '/account';
}
...
function clientIp(req) {
  // Use a platform-trusted source for the throttle key. The LEFTMOST x-forwarded-for
  // token is client-supplied and fully spoofable (a fresh random XFF per request would
  // rotate the throttle bucket and defeat rate limiting), so prefer Vercel's x-real-ip,
  // then the RIGHTMOST (closest-hop) XFF entry, then the socket address.
...
function blankUser(email) {
  ...
  const { newAccountId } = require('./accountId');
  return {
    email,
    id: newAccountId(), created_at: now, name: null, stripe_customer_id: null,
...
async function setSession(res, email, stateVersion, { kv, req } = {}) {
  // arc A3: every sign-in is REGISTERED so /app/account can list and revoke
  // it. kv/req are optional so old tests keep passing; without them the
  // session is legacy-shaped (no sid), which requireAuth accepts by design.
  if (kv) {
    const { issueSession } = require('./sessions');
    const { cookie } = await issueSession({ kv, email, stateVersion,
...
function makeRegisterHandler({ kv }) {
  return async (req, res) => {
    ...
    await setSession(res, email, 1, { kv, req });
    return res.status(200).json({ ok: true, redirect: landing(req) });
  };
}
...
function makeLoginHandler({ kv }) {
  return async (req, res) => {
    ...
    await setSession(res, email, user.state_version || 1, { kv, req });
    return res.status(200).json({ ok: true, redirect: landing(req) });
  };
}
...
function makeSetPasswordHandler({ kv }) {
  return async (req, res) => {
    ...
    await setSession(res, email, user.state_version, { kv, req });
    return res.status(200).json({ ok: true, redirect: landing(req) });
  };
}
