...
// Where to send them once they are in. /app unless they were bounced here from
// somewhere else, in which case back to that (QA finding 46: an invitation link
...
function landing(req) {
  return safeNext(req && req.body && req.body.next) || '/app';
}
...
async function setSession(res, email, stateVersion, { kv, req } = {}) {
  // arc A3: every sign-in is REGISTERED so /app/devices can list and revoke
