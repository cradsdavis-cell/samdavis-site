 ...
 */
// `billing` on the app token is now HAS A CARD, not a radio the user flipped
// (Sam, 2026-08-26). The worker's gate reads this claim, so the thing it gates
// on and the thing a customer can actually be charged with are the same fact.
function directoryFor(user, { fetcher = fetch, baseUrl = DIRECTORY_URL } = {}) {
  ...
  let token = null;
  const bearer = () => {
    if (!token) token = mintAppToken({ email, stateVersion: user.state_version || 1, accountId: user.id, billingEnabled: !!user.has_card });
    return token;
  };
  ...
  async function stageGrant({ mineralId, email: subject, role, action }) {
    let r;
    try {
      const t = mintAppToken({ email, stateVersion: user.state_version || 1, accountId: user.id, billingEnabled: !!user.has_card, scope: 'manage' });
  ...
  return {
    ...
    emailHash: eh,
    /**
     * What this account has run up this month, as the cockpit last published it.
     * ARREARS, so there is no balance: an account accrues a month and is charged
     * for it on the 1st.
     *
    ...
    async balance() {
      ...
      return {
        ok: true,
        month: r.body.month || '',
        accruedCents: Number(r.body.accrued_cents) || 0,
        perDayCents: Number(r.body.per_day_cents) || 0,
        lastSettledMonth: r.body.last_settled_month || null,
        lastSettledCents: Number(r.body.last_settled_cents) || 0,
        arrearsCents: Number(r.body.arrears_cents) || 0,
        charging: r.body.charging === true,
    ...
    async adminAll() {
      let r;
      try {
        const t = mintAppToken({ email, stateVersion: user.state_version || 1, accountId: user.id, billingEnabled: !!user.has_card, scope: 'admin' });
