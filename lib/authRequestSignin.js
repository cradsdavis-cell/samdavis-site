...
function makeHandler({ kv, resend }) {
  return async function handler(req, res) {
    ...
    // Carry the destination into the mail, so somebody who asks for a sign-in
    // link FROM an invitation lands back on the invitation rather than on the
    // account page wondering what happened to it (QA finding 46). Validated
    // here and again at redirect time in authVerifyToken.js.
